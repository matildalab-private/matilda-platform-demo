...
def preprocess(img, output, class_names) :

    boxes, objectness, classes, nums = output
...
flags.DEFINE_string('dataset', './brain_dataset/axial_brain_train.tfrecord', 'path to dataset')
flags.DEFINE_string('val_dataset', './brain_dataset/axial_brain_val.tfrecord', 'path to validation dataset')
flags.DEFINE_boolean('tiny', False, 'yolov3 or yolov3-tiny')
flags.DEFINE_string('weights', './checkpoints/yolov3.tf',
                    'path to weights file')
flags.DEFINE_string('classes', './brain_dataset/class.txt', 'path to classes file')
flags.DEFINE_string('output', './weights', 'path to output model file')
...
flags.DEFINE_float('decay', 0.00005, 'weight_decay')
...
def setup_model():
    ...
    optimizer = tf.keras.optimizers.Adam(lr=FLAGS.learning_rate, decay=FLAGS.decay)
    ...
    if FLAGS.mode == 'eager_tf':
        ...
        for epoch in range(1, FLAGS.epochs + 1):
            for batch, (images, labels) in enumerate(train_dataset):
                ...
                logging.info("epoch_{}_train_batch_{}, total_loss : {}, {}".format(
        ...
        callbacks = [
            ...
            ModelCheckpoint(f'{FLAGS.output}/yolov3_train_{epoch}.tf',
